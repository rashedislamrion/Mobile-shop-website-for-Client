...
async function main() {
        ...
        for (const module of modules) {
            for (const action of actions) {
                ...
                if (role.name === 'Admin') {
                    ...
                    if (['SALES', 'ORDERS', 'CUSTOMERS'].includes(module))
                        allowed = true;
                }
                else if (role.name === 'Technician') {
                    if (module === 'SALES' && (action === 'READ' || action === 'UPDATE'))
                        allowed = true;
                }
    ...
    const branches = [
        ...
        { name: 'Sylhet Warehouse', code: 'BR-SYL', type: 'WAREHOUSE', address: 'Zindabazar', city: 'Sylhet', phone: '01733333333' },
    ];
    let branchCount = 0;
    ...
    if (!existingSms)
        await prisma.smsConfig.create({ data: { provider: 'BulkSMSBD' } });
    ...
    if (!existingFirebase)
        await prisma.firebaseConfig.create({ data: {} });
