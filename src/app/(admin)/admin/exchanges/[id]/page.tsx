...
import { mockExchanges } from "@/lib/mock-data/exchanges";
...
export default function ExchangeDetailsPage({ params }: { params: { id: string } }) {
  ...
  const timelineSteps = [
    { label: "Requested", timestamp: exchange.date, isCompleted: true, isCurrent: false },
    { label: "Approved", timestamp: undefined, isCompleted: ["Approved", "Item Received", "Completed"].includes(exchange.status), isCurrent: exchange.status === "Approved" },
    { label: "Old Item Received", timestamp: undefined, isCompleted: ["Item Received", "Completed"].includes(exchange.status), isCurrent: exchange.status === "Item Received" },
    { label: "Completed", timestamp: undefined, isCompleted: exchange.status === "Completed", isCurrent: exchange.status === "Completed" },
  ];
      ...
      <div className="w-full lg:w-2/3 space-y-6">
        ...
        <div className="bg-white rounded-xl border border-slate-200 overflow-hidden">
          ...
          <div className="p-6 flex flex-col md:flex-row items-center gap-6">
            {/* Old Item */}
            <div className="flex-1 bg-white border border-slate-200 rounded-xl p-5 text-center shadow-sm">
              <div className="inline-flex items-center justify-center w-24 h-24 rounded-full bg-slate-100 mb-4 overflow-hidden relative border border-slate-200">
                <Image src={exchange.oldItem.image} alt="Old Item" fill className="object-cover" />
              </div>
              <h4 className="text-sm font-bold text-slate-400 uppercase tracking-wider mb-1">Old Item</h4>
              <p className="font-semibold text-slate-800 text-lg">{exchange.oldItem.name}</p>
              <p className="text-slate-500 mt-2 line-through decoration-red-400">৳{exchange.oldItem.price.toLocaleString()}</p>
            </div>
            ...
            {/* New Item */}
            <div className="flex-1 bg-emerald-50/30 border border-emerald-100 rounded-xl p-5 text-center shadow-sm">
              <div className="inline-flex items-center justify-center w-24 h-24 rounded-full bg-white mb-4 overflow-hidden relative border border-emerald-200">
                <Image src={exchange.newItem.image} alt="New Item" fill className="object-cover" />
              </div>
              <h4 className="text-sm font-bold text-emerald-600 uppercase tracking-wider mb-1">New Item</h4>
              <p className="font-semibold text-slate-800 text-lg">{exchange.newItem.name}</p>
              <p className="text-emerald-700 font-bold mt-2">৳{exchange.newItem.price.toLocaleString()}</p>
            </div>
          </div>
            ...
            <div className="text-right">
              {exchange.priceDifference > 0 ? (
                <>
                  ...
                  <p className="text-xl font-bold text-red-500">+৳{exchange.priceDifference.toLocaleString()}</p>
                </>
              ) : exchange.priceDifference < 0 ? (
                <>
                  <p className="text-sm text-slate-500">Refund to customer</p>
                  <p className="text-xl font-bold text-emerald-600">-৳{Math.abs(exchange.priceDifference).toLocaleString()}</p>
                </>
              ) : (
                <>
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <button onClick={() => handleAction("Item Received")} className="flex items-center gap-2 px-5 py-2.5 bg-blue-600 hover:bg-blue-700 text-white font-medium rounded-lg transition-colors text-sm">
            <PackageCheck className="w-4 h-4" /> Mark Old Item Received
          </button>
          <button onClick={() => handleAction("Complete")} className="flex items-center gap-2 px-5 py-2.5 border border-emerald-600 text-emerald-700 hover:bg-emerald-50 font-medium rounded-lg transition-colors text-sm">
        ...
        <div className="bg-white rounded-xl border border-slate-200 overflow-hidden p-5">
          ...
          <div className="space-y-3 text-sm">
            <div className="flex justify-between items-center pb-3 border-b border-slate-100">
              ...
              <span className="font-medium text-slate-800">{exchange.customerName}</span>
            </div>
            <div className="flex justify-between items-center pb-3 border-b border-slate-100">
              <span className="text-slate-500">Order ID</span>
              <Link href={`/admin/orders/${exchange.originalOrderId.replace('#', '')}`} className="font-medium text-emerald-600 hover:underline">
                {exchange.originalOrderId}
              </Link>
            </div>
            <div className="flex justify-between items-center pb-3 border-b border-slate-100">
              <span className="text-slate-500">Branch</span>
              <span className="font-medium text-slate-800">{exchange.branch}</span>
            </div>
            <div className="flex justify-between items-center">
              <span className="text-slate-500">Date Requested</span>
              <span className="font-medium text-slate-800">
                {new Date(exchange.date).toLocaleDateString('en-GB')}
              </span>
            </div>
          </div>
        </div>
