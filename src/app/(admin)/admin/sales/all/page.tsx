...
import { mockAllSales, MockSale } from "@/lib/mock-data/sales/all";
...
export default function AllSalesPage() {
  ...
  const filteredData = useMemo(() => {
    ...
    if (filters.saleType) {
      result = result.filter((o) => o.saleType === filters.saleType);
    }
  ...
  const createActions = (): TableAction[] => [
    ...
    { label: "Print", icon: <Printer className="w-4 h-4" />, onClick: (row) => console.log("Print", row) },
  ];
  ...
  const columns: ColumnDef<MockSale>[] = [
    {
      accessorKey: "id",
      header: "ID",
