...
export default function ProfilePage() {
  const { customerName, login } = useMockAuth();
  ...
  const onSubmit = async (data: ProfileFormValues) => {
    ...
    login(data.fullName.split(" ")[0]); // Update mock auth name
      ...
      <div className="bg-white border border-slate-100 p-6 rounded-2xl shadow-sm flex flex-col sm:flex-row items-center gap-6">
        <div className="relative">
          <Avatar className="w-24 h-24 border-4 border-slate-50 shadow-sm">
            ...
            <AvatarFallback>{customerName?.charAt(0) || "U"}</AvatarFallback>
          </Avatar>
        ...
        <div className="flex flex-col items-center sm:items-start text-center sm:text-left">
          <h2 className="text-xl font-bold text-slate-900 mb-1">{customerName || "Customer Name"}</h2>
          <Badge variant="outline" className="bg-amber-50 text-amber-600 border-amber-200 gap-1 rounded-full">
